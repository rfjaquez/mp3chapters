...
def add_chapters(fi, fname):
    ...
    for chap in chapswk:
        ...
        tag.chapters[element_id].title = "{}".format(title)
        child_idsb.append(element_id)
        index += 1
    tag.table_of_contents.set(b"toc", toplevel=True, child_ids=child_idsb, description="Table of Contents")
    # list_chaps(tag)
    # print_chaps(tag)
    tag.save()
    ...
    with open(chaps_fname, "w", encoding="utf-8-sig") as f:
        ...
        for toc in tag.table_of_contents:
            for chap_id in toc.child_ids:
                ...
                if fmt == "=cue":
                    track += 1
                    seconds, milliseconds = divmod(chapter.times[0], 1000)
                    minutes, seconds = divmod(seconds, 60)
                    frames = round(int(milliseconds) * 0.075)
                    fmtstr = f"{int(minutes):02d}:{int(seconds):02d}:{int(frames):02d}"
...
def test_chaps(tag):
    ...
    tag.table_of_contents.set(
        b"toc", toplevel=True, child_ids=[b"chp1", b"chp2", b"chp3"], description="Table of Contents"
    )
...
def main():
    """Entry point"""
    args = docopt(__doc__, version="mp3chaps 0.5.0")
